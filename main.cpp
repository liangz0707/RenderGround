...
private:
	...
	/*!
	制定应用程序和Vulkan库的连接
	@应用程序的信息 VkApplicationInfo
	@创建Instance VkInstanceCreateInfo
	@验证层 validationLayers
	@创建和检查扩展 glfwGetRequiredInstanceExtensions
	*/
	void createInstance()
	{
	...
	*/
	bool isDeviceSuitable(VkPhysicalDevice device) {
		...
		bool extensionsSupported = checkDeviceExtensionSupport2(device);
	...
	bool checkDeviceExtensionSupport2(VkPhysicalDevice device) {
	...
	*/
	...
	/*!
	给设备评分
		设备属性： name, type and supported Vulkan version
		设备特性： texture compression, 64 bit floats and multi viewport rendering。
	*/
	...
	/*!
	创建逻辑渲染设备：
		@确定需要使用的Queue：queueCreateInfos
		@从创建到的设备中获取提交命令的Queue
	*/
	...
	/*!
	创建buffer，无论什么设置方式都无法控制何时将数据写入GPU的buffer中，图像API的说明文件中指出
	向GPUbuffer传递数据是在后台进行的，我们只知道会在下一次调用vkQueueSubmit之前发生。
		@创建bufferInfo
		@确定需要内存
		@准备内存申请Info：内存类型和尺寸
		@将申请好的内存和buffer绑定
	*/
	...
	/*！
	创建让Vulkan在Windows显示的窗口，因为Vulkan是平台无关的
	*/
	void createSurface() {
	...
	*/
	...
	/*！
	明确指定Buffer(由SwapChain管理)
		@具体的SwapChain的信息核对：SwapChainSupportDetails
		@具体SwapChain图像格式颜色空间的选择
		@Presentation Mode
		@Swap extent：图像大小 resolution of the swap chain images 
		@最终根据指定参数(VkSwapchainCreateInfoKHR)创建SwapChain
		@SwapChain的用处Image Usage： specifies what kind of operations we'll use the images in the swap chain for
	...
	*/
	...
	/*!
	通过glfw平台相关组件创建窗口，
	去掉对Opengl的依赖：GLFW_NO_API
	暂时关闭窗口尺寸变化：GLFW_FALSE
	...
	*/
	...
	/*!
	使用任何VkImage的时候需要为其创建VkImageView。
	它描述了如何访问图像以及访问图像的哪一部分
	for example if it should be treated as a 2D texture depth texture without any mipmapping levels.
	为每一个SwapChain当中的Image创建ImageView这样才能够作为RenderTarget
	*/
	...
	/*!
	renderpass当中用到的attachments需要封装到framebuffer当中才能和renderpass绑定。 
	每一个Framebuffer对应了一个完整的RenderPass。
	*/
	void createFramebuffers() {
		...
		for (size_t i = 0; i < swapChainImageViews.size(); i++) {
			...
			// 绑定attachment对应RenderPass中VkAttachmentDescription的数量
	...
	*/
	...
	/*!
	从Shader加载开始创建图像管线。实际上是为RenderPass中的某个Subpass创建一个pipeline
	@在渲染中使用Shader要先把Shader封装到ShaderModule当中
	@Shader stages: 为了使用Shader，要把他放在特定的Stage当中。
	@Fixed-function state:配置管线的固定功能
	@配置Dynamic state
	...
	*/
	...
	@所有attachment的加载、渲染结束的设置
	@所有Subpasses的设置,指明其中要用的attachment。
	@RenderPassCreate的设置。
	*/
	...
	/*!
	创建命令池管理内存，用于管理buffer和commandebuffer。
	...
	*/
	...
	/*!
	为每一个Framebuffer穿件一个Commandbuffer
	@声明一个AllocateInfo
	@分配所有的Commandbuffer
	@为每一个Framebuffer创建Commandbuffer中的命令
	*/
	...
	*/
	...
	/*!
	创建同步GPU-GPU(主要针对SwapChain的)的信号和CPU-GPU(主要针对不同的pass之间的)同步信号
	*/
	...
	/*!
	DescriptorSetLayout指定了pipeline需要访问的类型。
	*/
	...
	*/
	...
	/*!
	要将图像从buffer拷贝到Image需要给图像设置正确的layout.
	用commandbuffer执行一个layout设置操作。
	*/
	...
	@使用第三方的图像库加载图像。
	@创建主机端的stagingbuffer
	@创建ImageInfo指定Image信息。
	@创建Image
	@创建Image的memory
	@转换Image的layout，成传输目的
	@将buffer内容copy到Image
	@装换Image的layout，成Shader可读
	*/
	...
	/*!
	创建Vertexbuffer，无论什么设置方式都无法控制何时将数据写入GPU的buffer中，图像API的说明文件中指出
		向GPUbuffer传递数据是在后台进行的，我们只知道会在下一次调用vkQueueSubmit之前发生。
		使用Stagingbuffer的原因是Host可以访问buffer在GPU执行渲染的时候效率不高。
	@创建bufferInfo
	@确定需要内存
	@准备内存申请Info：内存类型和尺寸
	@将申请好的内存和buffer绑定
	@将buffer的内存映射到可访问的CPU内存，然后才能进行填充，然后解绑
	*/
	...
	void createUniformBuffers()
	{
		...
		preEntityUniformBuffers.resize(swapChainImages.size());
		preEntityUniformBuffersMemory.resize(swapChainImages.size());
	...
	*/
	...
	/*!
	将所有操作合并，这些操作是一个函数中调用的，但是是异步执行的：
	@从swap chain当中获取一个图像。
	@将这个图像作为framenbufer当中的attachment执行Commandbuffer。
	@将图像返回到SwapChain进行显示。
	*/
	void drawFrame() {
	...
	void updateUniformBuffer(uint32_t currentImage) {
		...
		*/
		...
		vkMapMemory(device, preEntityUniformBuffersMemory[currentImage], 0, sizeof(eubo), 0, &data);
		memcpy(data, &eubo, sizeof(eubo));
		vkUnmapMemory(device, preEntityUniformBuffersMemory[currentImage]);
	}
